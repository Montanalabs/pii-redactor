...
Untrusted input is a document; it can only ever become one of a fixed set of decisions over a
...
- **Closed type:** `type Decision = Redact(FieldKind) | Pass` where `FieldKind` is closed
- **Sink / capability:** `grant redact` → `privileged { redact(d) }`
- **Consequence axes:** Trust

## Run the demo

```sh
examples/apps/pii-redactor/demo.sh
```
...
- `pii-redactor_safe.wave` — the correct design.
- `pii-redactor_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
